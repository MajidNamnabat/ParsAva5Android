...
public class SeekBarPreference extends DialogPreference implements SeekBar.OnSeekBarChangeListener
{
    ...
    private void persistSettings(int progress) {
        ...
        callChangeListener(text);
        if (shouldCommit()) {
            final Context storageContext = PreferenceStorage.resolveStorageContext(getContext());
    ...
    protected void onSetInitialValue(boolean restorePersistedValue, Object defaultValue) {
        final Context storageContext = PreferenceStorage.resolveStorageContext(getContext());
        final SharedPreferences prefs = PreferenceStorage.getDefaultSharedPreferences(storageContext);
        final int fallback = defaultValue instanceof Integer ? (Integer) defaultValue : mDefaultValue;
        final String persisted = prefs.getString(getKey(), Integer.toString(fallback));
        try {
            mProgress = Integer.parseInt(persisted);
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser)
    {
        ...
        mValueText.setContentDescription(text);
