...
public class TtsService extends TextToSpeechService {
    ...
    public void onCreate() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N)
            mStorageContext.moveSharedPreferencesFrom(this, this.getPackageName() + "_preferences");
        initializeTtsEngine();
    ...
    private void initializeTtsEngine() {
        if (mEngine != null) {
            mEngine.stop(SpeechSynthesis.StopReason.SERVICE_SHUTDOWN,
                    "notUsed.TtsService.initializeTtsEngine",
                    "engineRefresh");
            mEngine = null;
        }
        final AssetManager am = this.getAssets();
        final String pck_name = getApplicationContext().getPackageName();
        mEngine = new SpeechSynthesis(mStorageContext, mSynthCallback, pck_name, am);
        TextToSpeech sd;
