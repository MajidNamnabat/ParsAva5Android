...
- **Removed components** – MBROLA and the legacy NLP library have been removed from this project.
- **Persian synthesis stack** – MBROLA has been removed. Persian output now uses the ONNX path (`elnaz.onnx`) only, tuned for very rapid Google TalkBack speech rates.
...
- **ExtendedApplication** – MultiDex-aware `Application` that initializes user dictionary storage, tracks engine handle references via `Preferences`, and exposes architecture detection helpers for loading the correct native libraries. Optimizes long-running sessions by resetting handles when counts drop to zero. The legacy NLP library path has been removed.
...
- **ONNX-tuned Persian synthesis** – The `elnaz.onnx` path is tuned for quick buffer generation so Persian output keeps pace with rapid TalkBack navigation.
- **Bounded audio queue for TalkBack** – Audio buffers flow through a capacity-capped queue that drops the oldest packets when rapid TalkBack speech floods the pipeline, keeping ONNX Persian and English workers responsive without spiking memory.
- **Reference counting** – Engine handle tracking in `ExtendedApplication` to release resources when no longer needed, preventing leaks during long sessions. The NLP library has been removed.
...
- **Preload ONNX Persian assets** – Warm up `elnaz.onnx` in `FaTts.Load(...)` before the first TalkBack utterance to remove cold-start lag.
...
- **Keep queue drain low-latency** – Emit PCM chunks immediately from ONNX output and avoid waiting for full utterances; this keeps TalkBack feedback responsive during fast flick navigation.
...
- **Goal** – Keep the English path responsive without regressing the ONNX-powered Persian pipeline that TalkBack users rely on for rapid speech. The checks below focus on `PreferencesChangeReceiver` applying English slider updates to `mEnTts.mConfiureParams`.
- **Setup**
...
- **Audible confirmation**
  ...
  - Flip back to Persian and trigger a sample to ensure ONNX Persian playback still sounds rapid and unchanged, preserving TalkBack pacing.
- **Acceptance notes**
  ...
  - Returning to Persian leaves ONNX responsiveness intact so fast TalkBack navigation is preserved.
